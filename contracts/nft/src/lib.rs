...
impl Nft {
    ...
    fn mint(&mut self, to: &ActorId, token_metadata: TokenMetadata) -> NftEvent {
        ...
        self.check_color_metadata(&token_metadata);
    ...
    /// Check for the Vara Color requirements
    fn check_color_metadata(&self, metadata: &TokenMetadata) {
        if metadata.saturation > 100 {
            panic!("Saturation too high");
        }
        if metadata.light > 100 {
            panic!("Light too high");
        }
        if metadata.color_mode > 2 {
            panic!("Color mode not recognized");
        }
    }
